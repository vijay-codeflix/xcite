    ...
    public function rules()
    {
        $rules = match (strtolower($this->method())) {
            'post' => [
                ...
                'phone_no' => 'required|digits_between:' . VALIDATION['PHONE_LENGTH'] . '|unique:employees,phone_no,NULL,id,deleted_at,NULL',
                'password' => 'required|string|min:' . VALIDATION['MIN_PASSWORD_LENGTH'],
            ],
            'patch', 'put' => [
                'name' => 'nullable|string|max:' . VALIDATION['MAX_STRING_LENGTH'],
                'phone_no' => 'nullable|digits_between:' . VALIDATION['PHONE_LENGTH'] . '|unique:employees,phone_no,' . $this->employee->id . ',id,deleted_at,NULL',
